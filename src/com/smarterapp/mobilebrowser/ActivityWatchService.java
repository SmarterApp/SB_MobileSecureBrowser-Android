...
public class ActivityWatchService extends Service 
{
	...
	private void checkSettings() {
		...
		try {
			// the function call setPrimaryClip crashes on this device model, so do not call
			if (!android.os.Build.MODEL.equals("SM-T580")) {
				mClipboardManager.setPrimaryClip(NEW_PLAIN_TEXT);
			}
		} catch(Exception ex)
		{
			Log.e(TAG, "Failed to clear clipboard");
		}
